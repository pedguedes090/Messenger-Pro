...
# Asm fields (file send coroutine) - classes5.dex
for fname in sorted(glob.glob('apkmess/extracted/classes*.dex')):
    fields = parse_dex(fname)
    for f in fields:
        if f['class'] == 'LX/Asm;':
            tname = f['type'].replace('/', '.').strip('L').rstrip(';')
            print("Asm.%s : %s in %s" % (f['name'], tname, fname))

print()
# Also check 8t6 (text send coroutine) for comparison
for fname in sorted(glob.glob('apkmess/extracted/classes*.dex')):
    fields = parse_dex(fname)
    for f in fields:
        if f['class'] == 'LX/8t6;':
            tname = f['type'].replace('/', '.').strip('L').rstrip(';')
            print("8t6.%s : %s in %s" % (f['name'], tname, fname))
