...
public abstract class BaseHook {
    ...
    public final void inject(OrcaGateway gateway) {
        if (mStateTracker.shouldNotApply()) {
            Logger.info("Hook " + getId().name() + " skipped: state=" + mStateTracker.getState());
            return;
        }
    ...
    public class TrackerMethodHook extends XC_MethodHook {
        ...
        // Cached reflection lookups to avoid per-call overhead
        private final Method mCachedBefore;
        private final Method mCachedAfter;
        ...
        public TrackerMethodHook(XC_MethodHook methodHook, boolean ignoreWorking) {
            ...
            mCachedBefore = resolveMethod("beforeHookedMethod");
            mCachedAfter = resolveMethod("afterHookedMethod");
        }
        ...
        private Method resolveMethod(String name) {
            try {
                Method m = mMethodHook.getClass().getDeclaredMethod(name, MethodHookParam.class);
        ...
        protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
            ...
            if (mCachedBefore == null) return;

            try {
                mCachedBefore.invoke(mMethodHook, param);
        ...
        protected void afterHookedMethod(MethodHookParam param) throws Throwable {
            if (mStateTracker.shouldNotApply()) return;
            if (mCachedAfter == null) return;

            try {
                mCachedAfter.invoke(mMethodHook, param);
