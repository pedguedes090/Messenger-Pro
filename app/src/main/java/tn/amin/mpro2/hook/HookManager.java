...
public class HookManager {
    ...
    private static class PreferencesHookStateTracker extends HookStateTracker {
        ...
        private volatile int mCachedState;

        public PreferencesHookStateTracker(SharedPreferences sharedPreferences, String key) {
            mSharedPreferences = sharedPreferences;
            mKey = key;
            mCachedState = sharedPreferences.getInt("HOOK/" + key, HookState.PENDING.getValue());
        }

        @Override
        public void updateState(HookState state) {
            int newValue = state.getValue();
            if (mCachedState == newValue) return;
            mCachedState = newValue;
            mSharedPreferences.edit()
                    .putInt("HOOK/" + mKey, newValue)
                    .apply();
        }

        @Override
        public HookState getState() {
            return HookState.fromValue(mCachedState);
        }
