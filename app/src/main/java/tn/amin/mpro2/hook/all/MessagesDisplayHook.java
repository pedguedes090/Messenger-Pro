...
public class MessagesDisplayHook extends BaseHook {
    ...
    protected Set<XC_MethodHook.Unhook> injectInternal(OrcaGateway gateway) {
        java.lang.reflect.Method decoderMethod = gateway.unobfuscator.getMethod(OrcaUnobfuscator.METHOD_MESSAGES_DECODER_DECODE);
        if (decoderMethod == null) {
            Logger.info("MessagesDisplayHook: decoder method not found, feature unavailable");
            return Collections.emptySet();
        }
        Logger.info("MessagesDisplayHook: hooking " + decoderMethod.getDeclaringClass().getName() + "." + decoderMethod.getName());
        Set<XC_MethodHook.Unhook> hooks = new java.util.HashSet<>();
        hooks.add(XposedBridge.hookMethod(
                decoderMethod,
            ...
            protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                ...
                try {
                    ...
                    Logger.error("MSGDISPLAY error: " + t.getMessage());
                }
            }
        })));
        return hooks;
